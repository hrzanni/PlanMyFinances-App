...
| 8 | Melhorias de julho (cobranças, cartões, pastas, transações, início, perfil) | `[x]` | `plan/08-melhorias-julho/` |
...
- [ ] **6.5 Analytics de produto** — instrumentar web + mobile (recomendação: PostHog free; ver notas no README da fase)
- [ ] **6.6 Cadastro e perfil ricos** — mais dados no cadastro (nascimento, gênero, …) + critérios de senha fraca/moderada/forte e boas práticas de auth
- [x] **6.7 Safe area no header mobile** — header próprio com SafeAreaView no stack Mais (header nativo fica sob a status bar com o edge-to-edge do SDK 53, expo/expo#36685)
- [x] **6.8 Ícone do app + favicon** — logo aplicada (assets/brand/): ícone + adaptive icon no app.json e favicon via apps/web/src/app/icon.png
...
- [x] **8.1 Cobranças** — editar status/recebido, virar receita em categoria especial · `01-cobrancas.md`
- [x] **8.2 Faturas → Cartões** — cartão como entidade, transações pendentes associadas a um cartão, bancos pré-selecionados · `02-faturas-cartoes.md`
- [x] **8.3 Pastas** — remover campo de emoji · `03-pastas.md`
- [x] **8.4 Transações** — subcategoria no form mobile (web já tem) + tags de categoria/subcategoria no Histórico e Início · `04-transacoes-subcategoria-tags.md`
- [x] **8.5 Início** — logo na sidebar web + gráfico de pizza por categoria no lugar do saldo acumulado · `05-inicio-logo-grafico.md`
- [x] **8.6 Perfil** — redesenho (avatar com iniciais + dados completos) e cadastro com nascimento/gênero/telefone/senha forte · `06-perfil.md` (foto adiada por decisão do Hugo; storage segue em aberto)
...
- 2026-07-13: Web em produção na Vercel (https://plan-my-finances-app-web.vercel.app, root directory apps/web, NEXT_PUBLIC_API_URL para o Render) — circuito completo: Neon + Render + Vercel + APK. Fix necessário: cookie de sessão com SameSite=None+Secure em produção (web e API em domínios distintos; Lax não atravessa cross-site). Limitação conhecida: Safari bloqueia cookie de terceiros mesmo com None — solução futura é domínio próprio com web/API como subdomínios. WEB_ORIGIN gerenciado pelo render.yaml (editar lá, não no painel).
...
- 2026-07-13: 8.1 concluído (branch `feat/fase8-cobrancas-cartoes`) — "Registrar recebimento" de cobrança na Opção B confirmada pelo Hugo: tabela `charge_payments` espelhando `fixed_expense_payments` (histórico + desfazer), receita `source: 'charge'` criada na mesma transação de banco na categoria de sistema "Cobrança" (`categories.isSystem`, criada idempotente no primeiro uso, update/delete bloqueados com FORBIDDEN e read-only nas telas com badge "sistema"); quitação total marca a cobrança como paga e o desfazer reverte para pendente; excesso sobre o restante vira BAD_REQUEST (RN-004, `applyChargePayment`/`revertChargePayment` puros em @pmf/core). Componente genérico de installment separado em cobrança×fatura nas duas plataformas (web: charge/invoice-form-fields; mobile: charge/invoice-form-modal + charge/invoice-card), liberando o 8.2 para evoluir só o lado de fatura. UI: "+ receber" abre diálogo/modal com valor, histórico e desfazer (alvo derivado do cache para o resumo atualizar ao vivo); badge "cobrança" no Histórico. Migration aplicada no Docker local; gates verdes (lint/typecheck/test 33 na API+core/build); web verificada em runtime no navegador (registrar com vírgula, desfazer, KPIs, badges) e bundle Metro compila com os módulos novos.
- 2026-07-13: 8.6 concluído (branch `feat/fase8-perfil`, worktree) — foto de perfil adiada pelo Hugo ("focar nas informações"); cadastro ganha data de nascimento, gênero e telefone (opcionais, colunas novas em `users` via migration 0001, `additionalFields` do Better Auth nas duas pontas) e critério de senha forte: `passwordStrength` puro em @pmf/core (fraca/moderada/forte, testes) com medidor visual no cadastro web/mobile e senha fraca barrada também no servidor (hook `before` do Better Auth em sign-up/reset). Perfil redesenhado nas duas plataformas: card de cabeçalho com avatar de iniciais (`initials` puro em @pmf/core) + edição de nome/nascimento/gênero/telefone via `users.updateProfile` novo (Zod `updateUserProfileInput`; `updateName` mantido para builds mobile antigos; teste de service cobrindo salvar/limpar/omitir campos). Gênero como text validado por Zod (sem pg enum, evita conflito com 8.1/8.2 em enums.ts). Gates verdes (lint/typecheck/test 21 API + 66 core/build); migration aplicada no Docker local; web verificada em runtime no navegador (perfil salva e persiste, medidor fraca→forte ao vivo, curl cobre updateProfile/gênero inválido/senha fraca 400/signup completo) e bundle Metro compila com os módulos novos. Teste de toque (chips, teclado) pendente de aparelho.
- 2026-07-13: 8.2 concluído (branch `feat/fase8-cobrancas-cartoes`, depois do 8.1 por compartilharem o componente de installment) — modelo 1b confirmado pelo Hugo: tabela `cards` (name + enum `bank_preset`), `invoices.cardId` e `transactions.cardId` FKs nullable com SET NULL (excluir cartão desvincula sem apagar), e qualquer transação (receita ou despesa) pode apontar para um cartão — `cards.list` devolve `txCount` para ver o cartão mais usado. Router `cards` CRUD; mutações de transação/fatura validam posse do cartão (`cardBelongsToUser`, cartão de outro usuário → NOT_FOUND). Bancos preset decididos pelo Hugo: Nubank, Inter, Banco do Brasil, Santander, Caixa + "outro"; em vez de assets de imagem, catálogo `BANK_PRESETS` em @pmf/core (monograma + cores da marca) renderizado pelo componente `BankLogo` nas duas plataformas. UI: tela Cartões (web `/cartoes` + item na sidebar; mobile `mais/cartoes` + item no menu) com CRUD e chips de banco com logo; select/chips de cartão no form de transação e no de fatura (escolher cartão preenche o nome); Faturas agrupadas por cartão com cabeçalho logo+nome e grupo "Sem cartão" (tabela web extraída em `invoice-table.tsx` pelo limite de 300 linhas). Migration 0002 aplicada no Docker local; gates verdes (test 36: 30 API + 6 cards novos); runtime: fluxo completo via tRPC (criar cartão, vincular despesa/receita/fatura, txCount, NOT_FOUND para cartão alheio), `/cartoes` e `/faturas` verificados visualmente no navegador (criar cartão com preset+logo, editar preset, despesa vinculada via select do form, fatura agrupada sob o cabeçalho do cartão, txCount), bundle Metro compila com os módulos novos.
- 2026-07-13: 8.4 concluído (branch `feat/fase8-transacoes-inicio`) — `listTransactions` resolve `categoryName`/`subcategoryName` via left join (Opção A decidida pelo Hugo; teste novo no service cobrindo com/sem categoria), `CategoryBadge` ("Categoria" ou "Categoria › Subcategoria") no Histórico web e no card "Últimas transações" da Início, e no mobile o `TxRow` ganha a mesma tag (cobre Início, Histórico e Pastas) e o form de transação ganha chips de subcategoria após escolher categoria, com reset ao trocar categoria/tipo espelhando o web. Gates verdes (lint/typecheck/test 16 na API/build); web verificada em runtime no navegador (badges na Início e no Histórico, sem tag quebrada quando sem categoria) e bundle Metro compila com os módulos novos.
- 2026-07-13: 8.5 concluído (mesma branch `feat/fase8-transacoes-inicio`) — logo "Apenas o P" na sidebar web ao lado do nome (decisão do Hugo: logo + nome; `icone-quadrado.png` achatado de 4,8MB para 2,3KB em `apps/web/public/logo-p.png`, contraste ok nos dois temas pois a sidebar é escura em ambos) e pizza de despesas por categoria no lugar do "Saldo acumulado por dia" nas duas plataformas (decisões do Hugo: só despesas, substituição de vez). Núcleo: `expenseByCategory` + `foldCategorySlices` (top 5 + "Outras", "sem categoria" sempre preservada) puras em @pmf/core com testes; `monthSummary` retorna `byCategory` via left join. Web: `pie-chart-by-category.tsx` (Recharts, donut + legenda com % e valor); mobile: `PieChart` SVG puro (arcos via Path) em `charts.tsx`. Paleta categórica nova compartilhada entre plataformas, ordem fixa validada p/ daltonismo (validador da skill dataviz) nos dois temas; cinza reservado p/ "sem categoria". `BalanceLineChart` removido do web (arquivo) e do mobile (função). Gates verdes; web verificada em runtime nos temas claro e escuro (screenshot headless); bundle Metro compila com os módulos novos. Teste de toque no Expo Go fica com o Hugo.
- 2026-07-14: Faturas v2 concluído (branch `feat/fase8-cobrancas-cartoes`) — evolução da 8.2: unificação Cartões→Faturas nas duas plataformas (item de nav "Cartões" removido; gestão de cartão passa a viver dentro de Faturas), hero mensal com barras por cartão (`InvoiceMonthHero` + `CardStrip`, clique no mês/cartão filtra a lista), fatura ganha categoria própria e primeiro vencimento na criação, pagamento por parcela em tabela nova `invoice_payments` (espelha `charge_payments`/`fixed_expense_payments`: histórico + desfazer) que gera despesa automática com a categoria e o cartão da fatura, `source: 'invoice'` e descrição `"<fatura> — parcela n/total"` (`InstallmentPaymentModal` pré-preenchido); faturas 100% pagas migram para uma seção "Faturas encerradas" recolhida; widget `InvoicesMonthWidget` na Início mostra o resumo do mês corrente. Núcleo: `deriveInstallments`/lógica de fatura em @pmf/core; migration 0003 (`invoice_payments` + `invoices.category_id`/`first_due_date`) aplicada no Docker local. Specs: `docs/superpowers/specs/2026-07-14-faturas-v2-design.md` + plano `docs/superpowers/plans/2026-07-14-faturas-v2.md`. Gates verdes na raiz (lint/typecheck/test 80 core + 47 API/build; `/cartoes` ausente do build web). E2E via curl ponta a ponta: criar cartão → fatura com categoria própria → registrar parcela 1 (despesa com type/source/categoria/cartão/descrição corretos, aparece em transactions.list) → parcela duplicada → CONFLICT, parcela 99 → BAD_REQUEST → parcelas 2 e 3 → status `pago`/amountPaid `750.00` → desfazer parcela 3 → status `pendente`/amountPaid `500.00` e despesa some de transactions.list → cleanup completo (estado final idêntico ao inicial). Bundle Metro compila com os módulos novos (`InvoiceMonthHero`, `CardStrip`, `InstallmentPaymentModal`, `InvoicesMonthWidget` presentes). Verificado visualmente no navegador: hero com barra vermelha de atraso e clique por mês, dialog pré-preenchido com aviso da despesa automática, toast "Despesa criada" com desfazer, badge FATURA no Histórico, widget "✓ Faturas do mês em dia" na Início, edição de parcela paga→pendente removendo a despesa. Review final da branch aprovado após dois ajustes: migration 0004 (drop da check legada `invoice_paid_le_total`, que estourava 500 ao pagar acima do valor nominal; teste de regressão novo) e invalidação de `cards` pós-pagamento para o txCount dos tiles atualizar na hora. Teste de toque no Expo Go fica com o Hugo.
- 2026-07-16: Faturas promovida a tab própria na barra inferior do mobile (5 tabs: Início, Histórico, Fixos, Faturas, Mais), saindo do stack "Mais" — arquivo movido de `app/mais/faturas.tsx` para `app/(tabs)/faturas.tsx` (git mv), título removido do mapa do stack, referência de rota do widget da Início atualizada. FR-151 emendada. Tag "Em breve" adicionada em Conexões nas duas plataformas (mesmo padrão de `soon: true` já usado em Agente), web e mobile.
