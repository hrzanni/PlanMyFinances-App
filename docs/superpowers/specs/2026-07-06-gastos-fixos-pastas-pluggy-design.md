...
- FR-151: Navegação web: sidebar escura com grupos — principal (Início, Histórico, Dashboard, Categorias, Pastas), "Contas" (Gastos Fixos, Cobranças, Faturas), "Automação" (Conexões, Agente), Sair. Mobile: 5 tabs (Início, Histórico, Fixos, Dash, Mais) com stack "Mais" (Pastas, Categorias, Cobranças, Faturas, Conexões, Agente, Sair).
